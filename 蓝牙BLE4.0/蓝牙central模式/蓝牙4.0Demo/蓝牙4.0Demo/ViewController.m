...
// 蓝牙4.0设备名
static NSString * const kBlePeripheralName = @"iPhone";
// 通知服务
...
//服务
static NSString *const imac_fuwu = @"9FA480E0-4967-4542-9390-D343DC5D04AE";
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error
{
    ...
    for (CBService *service in peripheral.services)
    {
        ...
        // 获取对应的服务
        if ([service.UUID.UUIDString isEqualToString:kService1UUID] || [service.UUID.UUIDString isEqualToString:kService2UUID])
        {
            // 根据服务去扫描特征
...
{
    if ([characteristic.UUID.UUIDString isEqualToString:kCharacteristicNotifyUUID]) {
        
        NSData *data = characteristic.value;
        NSString *time = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
        [self showMessage:[NSString stringWithFormat:@"接受到的时间戳: %@",time]];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error
{
    ...
    for (CBCharacteristic *characteristic in service.characteristics)
    {
        ...
        if ([characteristic.UUID.UUIDString isEqualToString:kCharacteristicReadWriteUUID])
        {
            // 写入数据
            [self showMessage:@"写入数据"];
...
//            }
            ...
            UIImage *image = [UIImage imageNamed:@"挂牌集"];
            
            NSData *data = UIImagePNGRepresentation(image);
            NSString *size = [NSString stringWithFormat:@"%zd",data.length];
            NSData *start = [size dataUsingEncoding:NSUTF8StringEncoding];
            [peripheral writeValue:start forCharacteristic:characteristic type:CBCharacteristicWriteWithResponse];
            
            Byte  *bytes = (Byte *)[data bytes];
            Byte * pt = bytes;
            NSMutableData *innerData = [NSMutableData dataWithCapacity:0];
            
            while (pt < bytes+data.length) {
                if ((bytes+data.length - pt)>30) {
                    
                    if (innerData.length > 0) {
                        [innerData replaceBytesInRange:NSMakeRange(0, innerData.length) withBytes:pt length:30];
                    }
                    else
                    {
                        [innerData appendBytes:pt length:30];
                    }
                    
                }
                else
                {
                    [innerData replaceBytesInRange:NSMakeRange(0, innerData.length) withBytes:pt length:bytes+data.length - pt];
                }
                
                [peripheral writeValue:innerData forCharacteristic:characteristic type:CBCharacteristicWriteWithResponse];
